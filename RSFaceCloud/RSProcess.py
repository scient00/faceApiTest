...
class RsFaceProcesses(Process):
    ...
    def __writeTmpData(self):
        try:
            ...
            confTempPath = TMP_PATH + '/processdata.json'

            if os.path.exists(confTempPath):
                with open(confTempPath, 'r') as rfile:
                    data = json.load(rfile)
                    self.__vStatDataResult.push(data)


            with open(confTempPath, 'w') as wfile:
                json.dump(self.__vStatDataResult.getDictTuple(),wfile)
            ProcessLock.release()
        except:
            logging.error('__writeTmpData[%d] error!!!'% self.__rsInput.getThreadId())
